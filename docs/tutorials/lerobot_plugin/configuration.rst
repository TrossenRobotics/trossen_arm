        ...
        #. Check the saved images in :guilabel:`outputs/captured_images` to identify which camera index corresponds to which physical camera (e.g. ``0`` for ``video0`` or ``1`` for ``video1``):
            ...
            .. note::
                ...
                *Left: Master Sensor with full ISP processing. Right: Slave Sensor optimized for depth.*
                ...
                Review the captured images in ``outputs/captured_images`` and select the video index that provides the properly processed RGB color output (brighter, color-corrected image).
